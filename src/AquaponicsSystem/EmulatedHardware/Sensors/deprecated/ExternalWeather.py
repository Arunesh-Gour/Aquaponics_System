...
if ("ExternalWeather" not in (IoTDevice.IoTDevice.deviceTypes[0]).keys()):
   IoTDevice.IoTDevice.deviceTypes[0]["ExternalWeather"] = [
      {
...
class ExternalWeather (IoTDevice.IoTDevice):
   def __init__ (self, deviceName=None):
      super().__init__(deviceType="ExternalWeather", deviceName=deviceName)
   ...
   def write (self, key, value=None):
      ...
      if (key in list(self.deviceAttributes.keys())[:3]):
         if (type(value).__name__ == 'bool'):
            ...
            raise TypeError("{0} requires 'bool' only".format(key))
      elif (key in list(self.deviceAttributes.keys())[3:4]):
      ...
      elif (key in list(self.deviceAttributes.keys())[4:5]):
         if (type(value).__name__ == 'float'):
